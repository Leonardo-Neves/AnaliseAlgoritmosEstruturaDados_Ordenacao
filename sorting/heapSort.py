...
def reDoMinimum(A, n, i):
    ...
    if smallest != i:
        A[i], A[smallest] = A[smallest], A[i]
        A = reDoMinimum(A, n, smallest)

    return A

@njit(parallel=True)
def buildMinimum(A, n):

    i = n / 2 - 1
    while i >= 0:
        A = reDoMinimum(A, int(n), int(i))
        i = i - 1

    return A

@njit(parallel=True)
def reDoMaximum(A, esq, dir):
    i = esq
    j = i * 2 + 1
...
@njit(parallel=True)
def buildMaximum(A, n):
    esq = int((n / 2) - 1)
    
    while (esq >= 0):
        A = reDoMaximum(A, esq, n-1)
        esq = esq - 1
...
def heapSort(A, minimum=False):
    ...
    if minimum:
        A = buildMinimum(A, len(A))
        
        i = len(A) - 1
        while i >= 0:
            A[0], A[i] = A[i], A[0]
            reDoMinimum(A, int(i), 0)
            i = i - 1
